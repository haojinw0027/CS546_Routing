...
class TripleModelGenerator:
    def __init__(self, base_model_name: str, expert_model_name: str, 
                 hf_token: str, base_port: int = 8000, expert_port: int = 8001, consult_model: str = 'base', alpha=0.5, beta=0.5):
    ...
    def generate_with_triple_model(self, prompt: str, max_new_tokens: int = 64, benchmark: str = "truthfulqa") -> Dict[str, Any]:
        ...
        while True:
            ...
            elif base_probability > self.alpha:
                next_token = base_token
                model_used = "base_more_confident"
                base_model_count += 1
                confidence = base_probability
            elif expert_probability > self.beta * base_probability:
                next_token = expert_token
                model_used = "expert_cosd"
                expert_model_count += 1
                confidence = expert_probability
            else:
                next_token = base_token
                model_used = "expert_not_cosd"
                base_model_count += 1
                confidence = base_probability
...
def main():
    ...
    if args.consult_model == 'base':
        if args.few_shot == 'yes':
            output_file = f'./result/{dataset_dir}/{MODEL_NAME_DICT[args.base_model]}/{args.split}/{MODEL_NAME_DICT[args.expert_model]}/cosd.json'
        else:
            output_file = f'./result/{dataset_dir}/zero_shot/{MODEL_NAME_DICT[args.base_model]}/{args.split}/{MODEL_NAME_DICT[args.expert_model]}/cosd.json'
