#!/usr/bin/env python3
"""
Multi-benchmark inference with triple model approach using vLLM:
Supports TruthfulQA, MMLU-STEM, and StrategyQA datasets.

1. Base model generates token with confidence
2. If confidence >= threshold, use base model token
3. If confidence < threshold, judge model (vLLM) decides:
   - Use base model token
   - Use expert model token  
...
def parse_arguments():
    ...
    parser.add_argument("--benchmark", type=str, choices=["mmlu_pro", "MATH", "supergpqa", "minerva", "com_hard_intervention"], default="mmlu_pro",
                        help="Benchmark to evaluate (mmlu_pro, MATH, supergpqa, minerva, or com_hard_intervention)")
    # Model arguments
    ...
    parser.add_argument("--exchange_threshold", type=float, default=0.001,
                        help="Exchange threshold for entropy calculation")
...
class TripleModelGenerator:
    def __init__(self, base_model_name: str, expert_model_name: str, 
                 hf_token: str, base_port: int = 8000, expert_port: int = 8001, consult_model: str = 'base', alpha=0.5, beta=0.5, exchange_threshold=0.001):
    ...
    def calculate_entropy(self, prompt: str, port: int, top_k: int = 20) -> float:
        """
    ...
    def generate_with_triple_model(self, prompt: str, max_new_tokens: int = 64, benchmark: str = "truthfulqa") -> Dict[str, Any]:
        ...
        while True:
            ...
            base_entropy = self.calculate_entropy(current_prompt, self.base_port)
            base_entropy /= math.log2(20)
            ...
            expert_entropy = self.calculate_entropy(current_prompt, self.expert_port)
            expert_entropy /= math.log2(20) 
            expert_token = self.expert_tokenizer.decode([expert_token_id], skip_special_tokens=True)
            if current_model == "base":
                if base_entropy > self.exchange_threshold:
                    current_model = "expert"
                    next_token = expert_token
                    model_used = "expert"
                ...
                if expert_entropy <= self.exchange_threshold:
                    current_model = "base"
                    next_token = expert_token
                    model_used = "expert"
                    expert_model_count += 1
            ...
            token_choices.append({
                ...
                "expert_confidence": expert_probability if expert_token!='' else 0.0,
            ...
            if len(next_tokens) < 10:
                ...
                next_tokens.pop(0)
                next_tokens.append(next_token)
            if len(next_tokens) == 10 and next_tokens[0] == next_tokens[1] == next_tokens[2] == next_tokens[3] == next_tokens[4] == next_tokens[5] == next_tokens[6] == next_tokens[7] == next_tokens[8] == next_tokens[9]:
                break
...
def process_single_item(item: Dict[str, Any], generator, args, benchmark: str) -> Dict[str, Any]:
    """
...
def main():
    ...
    if args.consult_model == 'base':
        if args.few_shot == 'yes':
            output_file = f'./result/{dataset_dir}/{MODEL_NAME_DICT[args.base_model]}/{args.split}/{MODEL_NAME_DICT[args.expert_model]}/r_stitch_vllm_{args.exchange_threshold}.json'
        else:
            output_file = f'./result/{dataset_dir}/zero_shot/{MODEL_NAME_DICT[args.base_model]}/{args.split}/{MODEL_NAME_DICT[args.expert_model]}/r_stitch_vllm_{args.exchange_threshold}.json'
